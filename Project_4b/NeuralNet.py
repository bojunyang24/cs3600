...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            updatedWeight = self.weights[i] + (alpha * inActs[i] * delta)
            newWeights += [updatedWeight]
            totalModification += abs(updatedWeight - self.weights[i])
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        """YOUR CODE"""
        outputs = [inActs]
        previousLayer = inActs
        for i in range(len(self.layers)):
            list = []
            for j in range(len(self.layers[i])):
                list += [self.layers[i][j].sigmoidActivation(previousLayer)]
            previousLayer = list
            outputs += [list]
        return outputs
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        deltaTemp = nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                        error += deltaTemp
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while avgWeightChange > weightChangeThreshold and iteration < maxItr:
        iteration += 1
        avgError, avgWeightChange = nnet.backPropLearning(examplesTrain, alpha)
    ...
    for example in examplesTest:
        ...
        for outputNodeNum in xrange(len(outputs[-1])):
            if int(round(outputs[-1][outputNodeNum])) != example[1][outputNodeNum]:
                errored = True
        if errored:
            testError += 1
        else:
