...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closestSquare = None
        minDist = -1
        for currGhostDistribution in livingGhostPositionDistributions:
            maxProbState = currGhostDistribution.argMax()
            distance = self.distancer.getDistance(maxProbState, pacmanPosition)
            if distance < minDist or minDist < 0:
                closestSquare = maxProbState
        bestAction = None
        closestDistance = -1
        for legalAction in legal:
            distance = self.distancer.getDistance(closestSquare, Actions.getSuccessor(pacmanPosition, legalAction))
