...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for position in self.legalPositions:
                trueDist = util.manhattanDistance(pacmanPosition, position) ## get true dist between pacmanc and ghost
                beliefs[position] = emissionModel[trueDist] * self.beliefs[position]
                # pA = self.beliefs
                # self.beliefs.normalize()
        allPossible = beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # for pastPos in self.legalPositions:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, pastPos))
        #     for newPos, p in newPosDist.items():
        #         newBeliefs[newPos] += self.getBeliefDistribution()[pastPos] * p
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for possiblePos in self.legalPositions:
            for i in range(self.numParticles / numPositions):
                self.particles = self.particles + [possiblePos]
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        particleTracker = util.Counter()
        if noisyDistance is None:
            for i in range(len(self.particles)):
                self.particles[i] = self.getJailPosition()
            particleTracker[self.getJailPosition()] = 1
        else:
            for particle in self.particles:
                particleDist = util.manhattanDistance(particle, pacmanPosition)
                particleTracker[particle] += emissionModel[particleDist]
            if (particleTracker.totalCount() == 0):
                self.initializeUniformly(gameState)
            else:
                particleTracker.normalize()
                newParticleList = []
                for x in range(self.numParticles):
                    newParticleList = newParticleList + [util.sample(particleTracker)]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            particleList = particleList + [util.sample(newPosDist)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # permutations of the legalpositions and ghosts
        permutations = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(permutations)
        self.particles = []
        numPositions = len(permutations)
        for product in permutations:
            for i in range(self.numParticles / numPositions):
                self.particles = self.particles + [product]
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        #     posCounter[particle] += prob
        # if all(v == 0 for v in posCounter.values()):
        #     self.initializeParticles()
        # else:
        #     self.particles = [util.sample(posCounter) for v in range(self.numParticles)]
        particleTracker = util.Counter()
        for particle in self.particles: # self.particle is a permutation of tuples
            probability = 1
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    particleGhostDist = util.manhattanDistance(pacmanPosition, particle[i])
                    probability = probability * emissionModels[i][particleGhostDist]
            particleTracker[particle] += probability
        reinitialize = True
        for v in particleTracker.values():
            if v != 0:
                reinitialize = False
        if reinitialize:
            self.initializeParticles()
        else:
            self.particles = [util.sample(particleTracker) for v in range(self.numParticles)]
